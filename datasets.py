...
def detrend_and_compare(data):
    ...
    - max_difference: 原始数据与还原数据之间的最大误�?
    """
    ...
    def detrend_data(data):
        ...
        # 对每个空间点的时间序列进行线性回归，提取趋势�?
        for i in range(space_x):
            for j in range(space_y):
                y = data[:, i, j]

                # 使用STL进行趋势分解
                stl = STL(y, seasonal=13, period=12)  # 可根据需要调�?seasonal
                result = stl.fit()
...
def fill_placeholder_with_mean(data, placeholder=-9999):
    ...
    for i in range(data.shape[-1]):
        ...
        if not np.any(placeholder_mask):
            continue

        valid = variable[~placeholder_mask]
        fill_value = float(np.mean(valid)) if valid.size > 0 else 0.0
        variable[placeholder_mask] = fill_value
        data[..., i] = variable
    return data
...
def read_era():
    ...
    # Keep compatibility with existing pipelines that expect qz_hp-1.npy.
    _np_save(QZ_HP_1, qz_htp)
...
def load_data():
    ...
    # Backward-compatible fallback if qz_hp is not pre-generated.
    if QZ_HP_1.exists():
        qz_hp = np.expand_dims(_np_load(QZ_HP_1), axis=-1)
    else:
        qz_hp = np.expand_dims(np.zeros_like(_np_load(QZ_HTP_1)), axis=-1)
